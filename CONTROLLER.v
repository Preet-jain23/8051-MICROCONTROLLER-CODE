...
//***************************************************************************************************************
localparam ES0=4'b0000,ES1=4'b0001,ES2=4'b0010,ES3=4'b0011,ES4=4'b0100,ES5=4'b0101,ES6=4'b0110,ES7=4'b0111,IES4=4'b1000,IES5=4'b1001,IES6=4'b1010,CJNE1 = 4'b1111; 
//reg [1:0]Current_Phase;
//reg [2:0]Current_State;
reg [3:0]Next_State;
reg [1:0]Next_Phase;
reg [7:0]CONTROLLER_OPCODE;
...
reg [2:0]MULTICYCLE_OPERATION;
reg CONTROLLER_A_DEINITIALIZE;
reg CONTROLLER_SRC1_DEINITIALIZE;
reg [7:0]CONTROLLER_TEMP_RAM_DATA_STORED;
reg [7:0]TEMP_ROM_DATA;
reg [15:0]DPTR;
//////ramanai
reg CONTROLLER_out_bit_data_temp;
reg CONTROLLER_RAM_AND_ROM_BOTH;
reg CALL_TYPE_INSTRUCTION;
///////
...
CONTROLLER_PC_TEMP = CONTROLLER_dest[11:0];
CONTROLLER_OPCODE=CONTROLLER_rom_data[7:0];
CONTROLLER_rom_rd=1'bx;
CONTROLLER_rom_addr=8'hxxxx;
CONTROLLER_A_DEINITIALIZE=1'b0;
...
MULTICYCLE_OPERATION[2:0]=3'B000;
CONTROLLER_SRC1_DEINITIALIZE=1'B0;
CONTROLLER_RAM_AND_ROM_BOTH = 1'b0;
CALL_TYPE_INSTRUCTION = 1'b0;
...
`JB,`JBC : begin
CONTROLLER_rom_rd = 1'b1;
CONTROLLER_rom_addr = CONTROLLER_PC_TEMP;   // jump_target stores inside rom_out_data
CONTROLLER_SRC_3 = 8'h02;
CONTROLLER_rd=1'b1;
CONTROLLER_is_bit_addr = 1'b1;
CONTROLLER_addr = 8'h03;
CONTROLLER_ROM_NEEDED = 1'b0;
CONTROLLER_INDIRECT_ADDRESSING_USED = 1'b0;
...
`ADD_R ,`ADDC_R , `INC_R ,`SUBB_R , `DEC_R,`MOV_RA:
...
if((CONTROLLER_OPCODE & 8'hF8)== (`MOV_RA & 8'hF8) )begin
CONTROLLER_A_DEINITIALIZE = 1'b1;
end
...
`ANL_C ,`ADD_C ,`ADDC_C,`SUBB_C ,`ORL_C , `XRL_C ,`MOV_C:
...
if(CONTROLLER_OPCODE == `MOV_C) CONTROLLER_A_DEINITIALIZE = 1'b1;
Next_State=ES3;
Next_Phase=CS2;
end
...
`ADD_D, `ADDC_D ,`ANL_D  ,`ANL_AD ,`SUBB_D, `ORL_D ,`ORL_AD , `XRL_D , `XRL_AD,`MOV_D,`MOV_AD,`PUSH:
begin
CONTROLLER_rom_rd = 1'b1;
CONTROLLER_rom_addr = CONTROLLER_PC_TEMP;
CONTROLLER_SRC_3 = 8'h02;
...
if(CONTROLLER_OPCODE == `MOV_D)CONTROLLER_A_DEINITIALIZE = 1'b1;
if(CONTROLLER_OPCODE == `MOV_AD)CONTROLLER_SRC1_DEINITIALIZE = 1'b1;
Next_State=ES3;
Next_Phase=CS2;
end
...
CONTROLLER_rd = 1'b1;
CONTROLLER_SRC_3 = 8'h02;
CONTROLLER_RAM_AND_ROM_BOTH = 1'b1;
Next_State=ES3;
Next_Phase=CS2;
...
`ANL_B , `ANL_NB ,`CLR_B ,`ORL_B , `ORL_NB,`MOV_BC,`MOV_CB :begin
...
A=(CONTROLLER_OPCODE[5] == 0) ? ~A : 8'h00;
Next_State = ES6;
Next_Phase = CS2;
end
...
TEMPRARY_RAM_ADDRESS_STORED=CONTROLLER_out_data[7:0];
//A =(CONTROLLER_ROM_NEEDED == 1'b1) ? CONTROLLER_rom_data : CONTROLLER_out_data [7:0];
...
if(CONTROLLER_OPCODE == `CLR_B || CONTROLLER_OPCODE == `MOV_CB)
begin
 CONTROLLER_wr = 1'b1;
 CONTROLLER_rd=1'bx;
 CONTROLLER_in_bit_data = (CONTROLLER_OPCODE == `CLR_B)? 1'b0 : PSW[7];
 Next_State = ES6;
Next_Phase = CS2;
end

else if(CONTROLLER_OPCODE == `MOV_AI)
begin

CONTROLLER_addr =   CONTROLLER_out_data [7:0];
CONTROLLER_wr = 1'b1;
CONTROLLER_rd = 1'bx;
CONTROLLER_in_data = A;
...
CONTROLLER_TEMP_RAM_DATA_STORED = CONTROLLER_out_data;
...
CONTROLLER_rom_addr = CONTROLLER_PC_TEMP +1;

if(CONTROLLER_OPCODE == `LCALL)begin
CONTROLLER_addr = SP+1'b1;
SP = SP+1'b1;
...
if((CONTROLLER_OPCODE  & 8'h1F) == (`ACALL & 8'h1F))begin
CONTROLLER_addr = SP+1'b1;
SP = SP+1'b1;
CONTROLLER_in_data = CONTROLLER_dest;
CONTROLLER_wr = 1'b1;
...
`ADD_R ,`ADD_I, `ADD_D,`MOV_D,`MOV_RA:begin 
...
CONTROLLER_A_NOT_USED=1'b1;
PSW[7]=1'b0;
CONTROLLER_ALU_OPCODE=(CONTROLLER_OPCODE == `INC_R)?4'b0001:4'b0010;
end
...
CONTROLLER_SRC_1=(CONTROLLER_SRC1_DEINITIALIZE == 1'B1)?8'h00:CONTROLLER_out_data;
CONTROLLER_A_NOT_USED=1'b0;
CONTROLLER_ALU_OPCODE=`ALU_OR;
CONTROLLER_DIRECT_ADDRSSING_A_NOT_USED = 1'b1;
end
...
A = CONTROLLER_TEMP_RAM_DATA_STORED ^ A;
CONTROLLER_TEMP_RAM_DATA_STORED = A ^ CONTROLLER_TEMP_RAM_DATA_STORED;
A = CONTROLLER_TEMP_RAM_DATA_STORED ^ A;
CONTROLLER_A_NOT_USED = 1'b1;
Next_State = ES5;
Next_Phase = CS2;
...
CONTROLLER_ALU_OPCODE = 4'b0000;
PSW[7]=(CONTROLLER_OPCODE == `ORL_B ? PSW[7]:0)|CONTROLLER_out_bit_data;
...
CONTROLLER_SRC_2=((CONTROLLER_OPCODE & 8'b11111000) == (`DEC_R & 8'b11111000))?(CONTROLLER_out_data):((CONTROLLER_A_DEINITIALIZE == 1'b0)?((CONTROLLER_A_NOT_USED == 1'b1)? 1 : A):8'b0);
CONTROLLER_p_CY=PSW[7];

CONTROLLER_addr=(CALL_TYPE_INSTRUCTION == 1'b1)?CONTROLLER_addr:8'hxxxx;
CONTROLLER_rd=(CALL_TYPE_INSTRUCTION == 1'b1)?CONTROLLER_rd:1'bx;
CONTROLLER_rom_rd =(CALL_TYPE_INSTRUCTION == 1'b1)?CONTROLLER_rom_rd:1'b0;
...
CONTROLLER_in_data = (CONTROLLER_OPCODE == `PUSH ||((CONTROLLER_OPCODE & 8'hF8)  ==  (`XCH_D & 8'hF8)) )?
CONTROLLER_TEMP_RAM_DATA_STORED : CONTROLLER_dest;
CONTROLLER_addr=(CONTROLLER_OPCODE == `PUSH)?(CONTROLLER_dest):((CONTROLLER_INDIRECT_ADDRESSING_USED == 1'b1) ? TEMPRARY_RAM_ADDRESS_STORED : {3'b000,PSW[4],PSW[3],CONTROLLER_OPCODE[2:0]});
CONTROLLER_wr=1'b1;

if(CONTROLLER_OPCODE == `PUSH)
begin 
SP = CONTROLLER_dest;
end
...
CONTROLLER_in_data =(CONTROLLER_ROM_NEEDED == 1'b0) ? CONTROLLER_out_data :CONTROLLER_rom_data;
CONTROLLER_rom_rd=1'b0;
CONTROLLER_rd=1'bx;
CONTROLLER_addr=(CONTROLLER_RAM_AND_ROM_BOTH == 1'b1)?(CONTROLLER_rom_data):{3'b000,PSW[4],PSW[3],CONTROLLER_OPCODE[2:0]};
CONTROLLER_wr=1'b1;
...
CONTROLLER_in_data = CONTROLLER_dest;
CONTROLLER_addr = TEMPRARY_ROM_ADDRESS_STORED;
CONTROLLER_wr = 1'b1;

end

else if(CONTROLLER_ALU_OPCODE != 4'b0000) begin 
A=CONTROLLER_dest;
PSW[7]=CONTROLLER_CY;
PSW[6]=CONTROLLER_AX;
end
...
casex(CONTROLLER_OPCODE) 
`JBC : begin
CONTROLLER_out_bit_data_temp = 1'b0;
end 
endcase
Next_State=ES6;
Next_Phase=CS2;
...
ES6:
begin
PC[11:0]=(CALL_TYPE_INSTRUCTION == 1'b1)?{((((CONTROLLER_OPCODE & 8'h1F) == (`AJMP & 8'h1F ))
                                              ||((CONTROLLER_OPCODE & 8'h1F) == (`ACALL & 8'h1F )))?({PC[15:11],CONTROLLER_OPCODE[7:5]}): CONTROLLER_rom_data),CONTROLLER_PC_TEMP[7:0]}:CONTROLLER_PC_TEMP[11:0];
Next_State=ES7;	
Next_Phase=CS2;
end
